...
def draw_radial_velocity(iterations=1000):

    optimizer = GenOpti(mutation_size=0.25)
    optimizer.simulate_generations(iterations, p_m=0.1)

    #solution_vector = np.array([494.2, 14299, 5.7397, 0.2626, 8.3836, 1.0026]).reshape(1, 6) #solution du prof
    ...
    plt.savefig('results/data_fitting1_' + str(objective_value) + '.pdf')
    ...
    plt.legend(loc='best', prop={'size': 10})
    plt.savefig('results/data_fitting2_' + str(objective_value) + '.pdf')
